<template>
    <section>
        <div class="py-10">
            <div class="flex max-[400px]:flex-col gap-5 items-center justify-between mb-4">
                ...
                <div class="flex items-center justify-end h-12 mb-4 space-x-2">
                    <button ref="prevButton"
                        class="flex items-center mr-4 text-white duration-300 bg-primary-blue hover:opacity-80"></button>
                    <button ref="nextButton"
                        class="flex items-center text-white duration-300 bg-primary-blue hover:opacity-80"></button>
                </div>
            </div>
            <Swiper :modules="modules" :slides-per-view="1" :space-between="16" :loop="true" :centeredSlides="true"
                :autoplay="autoplayConfig" :navigation="navigation" class="mySwiper">
                <SwiperSlide v-for="(event, index) in news" :key="index">
                    <div class="h-[250px] max-[500px]:h-[200px] max-[400px]:h-[150px] sm:h-[300px] lg:h-[500px] xls:h-[700px] flex flex-col">
                        <img class="object-cover  object-center w-full h-full" :src="event.image" alt="">
                        <div class="absolute text-xs sm:text-base flex items-center fill-white gap-1 px-4 py-2 mb-3  font-bold top-10 left-0 bg-primary-blue text-white">
                            <svg class="w-3 h-3 sm:w-[18px] sm:h-[18px]" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 24 24">
                                <path
